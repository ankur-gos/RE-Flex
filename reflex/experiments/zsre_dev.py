...
ex = setup_experiment('RE-Flex zsre dev')
...
def conf():
    ...
    data_directory = os.path.join(os.environ['BASE_PATH'], 'data/zsre/dev') # Path to underlying data
    batch_size = 16
...
def main(model_dir, model_name, device, relations_filepath, data_directory, batch_size, must_choose_answer, word_embeddings_path, ls,  k):
    ...
    with open('zsre_tune.pkl', 'wb') as wf:
