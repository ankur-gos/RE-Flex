...
def run_thread(arguments):
    ...
    em, f1 = metrics.calculate_em_f1_base(arguments['target'], pred)
    msg += "\n" + return_msg
...
def decode_lm(model, template, subject, decode_to_end):
    IT = 0
    while True:
        if IT == 0:
            result = model.inferbase(template, subject, base.ROBERTA_MASK, 1)
            tok = result[0]['token_word_form']
            if tok.startswith('_'):
                # predicted a subword, just return nothing
                return ''
            current_result = (f'{template.replace("[Y]", tok)} [Y]', tok)
            IT += 1
        else:
            templ, currstr = current_result
            result = model.inferbase(templ, subject, base.ROBERTA_MASK, 1)
            tok = result[0]['token_word_form']
            if not tok.startswith('_') and not decode_to_end:
                # this is a new word, return the word
                return currstr
            # Going to cap at 20 inferences. Nothing in any of the datasets is larger than 20 subwords so EM and F1 will be approx 0.
            elif tok == '</s>' or IT > 20:
                return currstr

            templ = f'{templ.replace(" [Y]", tok[1:-1])} [Y]'
            new_currstr = f'{currstr}{tok[1:-1]}'
            current_result = (templ, new_currstr)
            IT += 1
